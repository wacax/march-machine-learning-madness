...
shinyServer(function(input, output) {
  ...
  output$myPlot <- renderImage({
    ...
    fn <- paste0(dataDirectory, 'sample_submission.csv')
    tgtseason <- 'S'
    solfn <- paste0(dataDirectory, 'solution.csv')
    outputName <- paste(substr(fn, 1, nchar(fn)-3), "png", sep="")
    submission<-read.csv(fn)
    teams<-read.csv(paste0(dataDirectory, 'teams.csv'))
    seeds<-read.csv(paste0(dataDirectory, 'tourney_seeds.csv'))
